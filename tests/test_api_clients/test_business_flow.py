...
class TestBusinessFlowAPIClient(object):
    ...
    def mock_session(self, request):
        with mock.patch(
            "algosec.api_clients.business_flow.BusinessFlowAPIClient.session"
        ) as mock_session:
            yield mock_session
    ...
    def test_get_network_service_by_name(
    ...
    def test_get_application_revision_id_by_name(
        self, client, mock_session, mock_check_response
    ):
        response = mock_session.get.return_value
    ...
    def test_search_network_objects(self, client, mock_session, mock_check_response):
        ...
        mock_session.get.assert_called_once_with(
            ...
            params={"address": "ip-or-subnet", "type": search_type.value},
        )
        mock_check_response.assert_called_once_with(response)
        assert result == search_result

    def test_search_network_objects__empty_search(
        self, client, mock_session, mock_check_response
    ):
        search_type = MagicMock()
        response = mock_session.get.return_value
    ...
    def test_get_network_object_by_name(
        self, client, mock_session, mock_check_response
    ):
    ...
    def test_get_network_object_by_name__support_buggy_response(
        self, client, mock_session, mock_check_response
    ):
        """Make sure that we support the return value of a list of one object"""
        network_object = {"name": "some-object-name"}
        ...
        assert (
            client.get_network_object_by_name("network-object-name") == network_object
        )

    def test_get_network_object_by_name__invalid_response_object(
        self, client, mock_session, mock_check_response
    ):
        """Make sure that we support the return value of a list of one object"""
    ...
    def test_create_network_object(self, mock_session, mock_check_response, client):
        ...
        response = mock_session.post.return_value
        result = client.create_network_object(
            network_object_type, "object-content", "object-name"
        )
        mock_session.post.assert_called_once_with(
    ...
    @mock.patch(
        "algosec.api_clients.business_flow.BusinessFlowAPIClient.search_network_objects"
    )
    @mock.patch(
    ...
    def test_create_missing_network_objects(
        ...
        def object_search(obj_string, search_type):
            return {
                ...
                "10.0.0.2": [{"name": "object-with-same-content-but-different-name"}],
                # Object with search result that it is already created on ABF
                "10.0.0.3": [{"name": "10.0.0.3"}],
            }[obj_string]
    ...
    @mock.patch(
        "algosec.api_clients.business_flow.BusinessFlowAPIClient.get_application_flows"
    )
    def test_get_flow_by_name(
        self, mock_get_application_flows, client, mock_session, mock_check_response
    ):
        flow1 = {"name": "flow1"}
        flow2 = {"name": "flow2"}
    ...
    def test_get_flow_by_name__flow_not_found(
        ...
        mock_get_application_flows.return_value = [{"name": "flow1"}, {"name": "flow2"}]
    ...
    @mock.patch(
        "algosec.api_clients.business_flow.BusinessFlowAPIClient.get_flow_by_name"
    )
    @mock.patch(
        "algosec.api_clients.business_flow.BusinessFlowAPIClient.delete_flow_by_id"
    )
    ...
    @mock.patch(
        "algosec.api_clients.business_flow.BusinessFlowAPIClient.create_missing_network_objects"
    )
    def test_create_application_flow(
        self, mock_create_missing_objects, client, mock_session, mock_check_response
    ):
        ...
        response = mock_session.post.return_value
        result = client.create_application_flow("app-revision-id", requested_flow)
        mock_session.post.assert_called_once_with(
        ...
        mock_create_missing_objects.assert_called_once_with(
            {"source1", "source2", "dest1", "dest2", "double-obj"}
        )
    ...
    def test_get_associated_applications(
        self, client, mock_session, mock_check_response
    ):
        response = mock_session.get.return_value
    ...
    def test_get_abf_application_dashboard_url(self, client):
        ...
        assert (
            dashboard_url
            == "https://testing.algosec.com/BusinessFlow/#application/<app-revision-id>/dashboard"
        )
